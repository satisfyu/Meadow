...
public class CookingCauldronBlock extends BlockWithEntity {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, ModBlockEntities.COOKING_CAULDRON_BLOCK_ENTITY.get(), (world1, pos, state1, be) -> be.tick(world1, pos, state1, be));
    }
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        if (random.nextDouble() < 0.1) {
            world.playSound(d, e, f, ModSounds.COOKING_CAULDRON.get(), SoundCategory.BLOCKS, 1.0f, 1.0f, false);
        }
