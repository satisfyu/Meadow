...
public class CookieCowEntity extends CowEntity {
    ...
    public CowEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
        return EntityRegistry.COOKIE_COW.get().create(serverWorld);
    }
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (((bl = itemStack.isOf(Items.BUCKET)) || itemStack.isOf(ObjectRegistry.WOODEN_BUCKET.get())) && !this.isBaby()) {
            ...
            ItemStack itemStack2 = ItemUsage.exchangeStack(itemStack, player, bl ? Items.MILK_BUCKET.getDefaultStack() : ObjectRegistry.WOODEN_BUFFALO_MILK.get().getDefaultStack());
