...
public class ArmorRegistry {
    ...
    public static <T extends LivingEntity> void registerHat(Map<Item, EntityModel<T>> models, EntityModelLoader modelLoader) {
        models.put(ObjectRegistry.FUR_HELMET.get(), new FurArmorHat<>(modelLoader.getModelPart(FurArmorHat.LAYER_LOCATION)));
    }

    public static <T extends LivingEntity> void registerArmor(Map<FullCustomArmor, Pair<BipedEntityModel<T>, BipedEntityModel<T>>> models, EntityModelLoader modelLoader) {
        models.put(new FullCustomArmor(ObjectRegistry.FUR_CHESTPLATE.get(), ObjectRegistry.FUR_LEGGINGS.get(), ObjectRegistry.FUR_BOOTS.get(), new MeadowIdentifier("textures/models/armor/fur.png")),
                new Pair<>(
                        new FurArmorOuter<>(modelLoader.getModelPart(FurArmorOuter.LAYER_LOCATION)),
                        new FurArmorInner<>(modelLoader.getModelPart(FurArmorInner.LAYER_LOCATION))
                )
        );
    }
}