...
public class CheeseRackBlock extends Block {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (player.isSneaking() && state.get(STAGE) > 0) {
            ...
            player.giveItemStack(new ItemStack(ModBlocks.CHEESE_BLOCK));
        } else if (stack.getItem() == ModBlocks.CHEESE_BLOCK.asItem() && state.get(STAGE) < MAX_STAGE) {
            world.setBlockState(pos, state.with(STAGE, state.get(STAGE) + 1), 3);
    ...
    public List<ItemStack> getDroppedStacks(BlockState state, LootContext.Builder builder) {
        ...
        if(amount > 0) list.add(new ItemStack(ModBlocks.CHEESE_BLOCK, amount));
        return list;
    }
